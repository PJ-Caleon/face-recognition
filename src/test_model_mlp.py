...
#MLP
mlp = Interpreter(model_path="mlp_classifier.tflite")
mlp.allocate_tensors()
input_mlp = mlp.get_input_details()
output_mlp = mlp.get_output_details()
...
def find_match(embedding):
    ...
    mlp.set_tensor(input_mlp[0]['index'], embedding)
    mlp.invoke()
    preds = mlp.get_tensor(output_mlp[0]['index'])[0]

    class_id = np.argmax(preds)
    distance = preds[class_id]
